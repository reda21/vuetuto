    ...
    <div class="container mx-auto px-4">
      <div class="flex h-16 items-center justify-between">
        <div class="flex items-center space-x-8">
          ...
          <div class="hidden space-x-4 md:flex md:gap-2">
            <NuxtLink v-for="link in navLinks" :key="link.path" :to="link.path" class="navbar-link"
      ...
      <Transition enter-active-class="transition duration-300 ease-out"
        ...
        <div v-if="isOpen" class="submenu">
          <NuxtLink v-for="link in navLinks" :key="link.path" :to="link.path" class="submenu-link"
            exactActiveClass="active" @click="isOpen = false">
            {{ link.name }}
          </NuxtLink>
        </div>
      </Transition>
